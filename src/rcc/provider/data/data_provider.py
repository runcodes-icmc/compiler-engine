...
class DataProvider:
    ...
    Implementations access the database asynchronously and hold a connection
    pool that must be opened before use (and closed on shutdown).
    """

    async def open(self) -> None:
        """Open any underlying resources (e.g. a connection pool)."""

    async def close(self) -> None:
        """Close any underlying resources (e.g. a connection pool)."""
    ...
    async def claim_commit(self, _commit: Commit) -> bool:
        ...
        consumer had already taken it. The default implementation always
    ...
    async def release_commit(self, _commit: Commit) -> None:
        ...
        Only the consumer that holds the claim may call this. The default is a
